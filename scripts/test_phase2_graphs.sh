...
PARITY_SPK_LOG="$(mktemp "${TMPDIR:-/tmp}/thvm_phase2_parity_spk.out.XXXXXX")"
PARITY_LBK_LOG="$(mktemp "${TMPDIR:-/tmp}/thvm_phase2_parity_lbk.out.XXXXXX")"
...
if ! ./scripts/test_phase1_reduce_parity.sh bin/test_tiny_linear_bias_keep test/test_tiny_linear_bias_keep.m >"$PARITY_LBK_LOG" 2>&1; then
  cat "$PARITY_LBK_LOG"
  exit 1
fi

python3 scripts/check_phase2_graphs.py "$SP_DIR"  --expect-log 1 --expect-kernels 1 --min-kernels 1
python3 scripts/check_phase2_graphs.py "$LB_DIR"  --expect-log 1 --expect-kernels 2 --min-kernels 2
python3 scripts/check_phase2_graphs.py "$SPK_DIR" --expect-log 0 --expect-kernels 1 --min-kernels 1
