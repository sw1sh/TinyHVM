...
inet_step:
    ...
    switch (tag) {
        case TAG_TOP: {
            ...
            // Dispatch
            if (uop == UOP_MM) {
                // Keep MPS matmul for now — decomposition breaks backward.
                // TODO: decompose MM at interact time with proper forward tensor
                // provenance so GRAD handler can still use specialized MM backward.
