...
static void test(const char *dev){
    ...
    u32 OH=20; // 28-5+1=24, 24-5+1=20
    u32 ff=4*OH/2*OH/2; Term lw=mkw(ctx,SHAPE(ff,10),ff),lb=mkz(ctx,10);
    thvm_set_requires_grad(ctx,cw1);thvm_set_requires_grad(ctx,cb1);
    thvm_set_requires_grad(ctx,cw2);thvm_set_requires_grad(ctx,cb2);
    ...
    BNResult bn=batchnorm_forward(ctx,h,bn_g,bn_b,bn_rm,bn_rv,BS,4,OH,OH,1);
    ...
    #define NP 4
    Term params[NP]={cw1,cb1,cw2,cb2};u32 psz[NP]={4*25,4,4*4*25,4};
