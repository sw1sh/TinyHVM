...
static void test(const char *dev){
    ...
    Term cw1=mkw(ctx,(Shape){.dims={32,1,4,4},.rank=4},25),cb1=mkz(ctx,32);
    Term cw2=mkw(ctx,(Shape){.dims={32,32,4,4},.rank=4},800),cb2=mkz(ctx,32);
    Term bn_g=mkones(ctx,32),bn_b=mkz(ctx,32),bn_rm=mkz(ctx,32),bn_rv=mkones(ctx,32);
    u32 ff=32*11*11; Term lw=mkw(ctx,SHAPE(ff,10),ff),lb=mkz(ctx,10);
    ...
    BNResult bn=batchnorm_forward(ctx,h,bn_g,bn_b,bn_rm,bn_rv,BS,32,22,22,1);
    ...
    #define NP 4
    Term params[NP]={cw1,cb1,cw2,cb2};u32 psz[NP]={32*16,32,32*32*16,32};
