...
static void test(const char *dev) {
    ...
    TinyHVM*ctx=thvm_init(dev); u32 BS=1;
    ...
    #define NP 4
    Term params[NP]={cw1,cb1,cw3,cb3};
    u32 psz[NP]={32*25,32,64*32*9,64};
    for(u32 i=0;i<NP;i++) thvm_set_requires_grad(ctx,params[i]);
    thvm_set_requires_grad(ctx,cw2);thvm_set_requires_grad(ctx,cb2);
    thvm_set_requires_grad(ctx,cw4);thvm_set_requires_grad(ctx,cb4);
    thvm_set_requires_grad(ctx,bn1_g);thvm_set_requires_grad(ctx,bn1_b);
    thvm_set_requires_grad(ctx,bn2_g);thvm_set_requires_grad(ctx,bn2_b);
    ...
    u32 nw=ctx->tensor_count; u32 bi=0;
    ...
    printf("%-6s BS=%u: ",dev,BS);
    const char*n[NP]={"cw1","cb1","cw3","cb3"};
