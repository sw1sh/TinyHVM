...
static void test(const char *dev) {
    ...
    // Force BN to materialize BEFORE pool
    thvm_reduce(ctx,bn.output);
    f32*bnout=thvm_to_host(ctx,bn.output);
    u32 bnn=BS*Cout*OH*OH;
    double bnnorm=0;for(u32 i=0;i<bnn;i++)bnnorm+=bnout[i]*bnout[i];
    printf("%-6s: bn norm=%.4f", dev, sqrt(bnnorm));
    // Now apply pool
    h=bn.output;
    h=thvm_maxpool2d(ctx,h,(u32[]){2,2},(u32[]){2,2});
    thvm_reduce(ctx,h);
    f32*pout=thvm_to_host(ctx,h);
    u32 pn=BS*Cout*OH/2*OH/2;
    double pnorm=0;for(u32 i=0;i<pn;i++)pnorm+=pout[i]*pout[i];
    printf(" pool norm=%.4f pool[0..3]=[%.4f,%.4f,%.4f,%.4f]\n",
        sqrt(pnorm),pout[0],pout[1],pout[2],pout[3]);
    thvm_free(ctx);
}
