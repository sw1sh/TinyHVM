// test_dupmul_in_ref_grad_outside.m — REF body now contains expand+sum+
// user-DUP+MUL (returns sq[1,N]). Sum+grad OUTSIDE the REF.
//
// Differentiates from:
//   test_sum_in_ref_dup_outside (PASSES): REF returns pooled, DUP/MUL/grad outside.
//   test_sum_dupmul_grad_ref (FAILS): everything (incl. grad) inside REF body.
//
// If PASSES: bug requires grad-inside-REF. DUP/MUL inside REF with grad
//            outside still works after the y_shape fix.
// If FAILS:  bug is triggered by user DUP-of-ALO-SUM in backward, even
//            when grad is taken outside the REF. Requires fixing the
//            DUP⊳ALO interaction itself, not just grad-inside.
...
int main(int argc, char **argv) {
    ...
    // REF body: λw_p. { pooled = sum(expand(reshape(w_p, [1,N])), [M,N], axis 0);
    //                   dup pooled into (dp0, dp1);
    //                   sq = dp0 * dp1;
    //                   return sq } — returns [1, N]
    ...
    Term sq_body = thvm_op(ctx, UOP_MUL, dp0, dp1);
    ...
    // Call REF, get sq. Then OUTSIDE REF: sum to scalar, grad.
