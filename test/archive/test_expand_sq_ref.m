// test_expand_sq_ref.m — forward with EXPAND but no matmul: is the bug
// triggered by EXPAND+self-MUL alone, or does it need the full
// matmul-decomp chain (reshape+expand+mul+sum+reshape)?
//
// Forward: w is [N]. Expand to [M, N]. Self-MUL. Sum. All through a
// REF + MAT-CTR defer body.
//
// loss = sum(expand(reshape(w, [1, N]), [M, N])^2)
//
// Expected: gy for expanded_w = 2 * expand(w). gy for w = sum_i(2 * w[j])
//           * M = 2 * M * w[j] per j.
//
// If PASSES: bug needs full matmul decomp (with SUM reduction). EXPAND
//            alone not enough.
// If FAILS: EXPAND + self-MUL through defer+REF is sufficient. Narrows
//           the bug to EXPAND commute under defer.
...
int main(int argc, char **argv) {
    ...
    f32 bd[N] = { 0.05f, -0.10f, 0.15f, 0.20f };  // dummy 2nd param
    ...
    Term b_grad;
    { Term bunused; thvm_dup(ctx, thvm_fresh_label(ctx), b_p, &bunused, &b_grad);
      (void)bunused; }

    // Forward: w -> reshape [1,N] -> expand [M,N] -> self-MUL -> sum.
    // b isn't used in forward; it's only a grad target so we can use
    // the 2-arity CTR bundle and MAT-CTR destructure.
    Term w_bc = thvm_expand(ctx, thvm_reshape(ctx, w_fwd, SHAPE(1, N)), SHAPE(M, N));
    Term sq = thvm_op(ctx, UOP_MUL, w_bc, w_bc);
    ...
    // Expected: gw[j] = 2 * M * w[j]
    f32 exp_gw[N];
    for (u32 j = 0; j < N; j++) exp_gw[j] = 2.0f * (f32)M * wd[j];
