// test_jit_dense.m — JIT replay: dense + softmax + CE loss, no BN
#include "../src/tinyhvm.c"
#include "../src/backend/cpu/_.c"
#ifdef __APPLE__
  #include "../src/backend/metal/_.m"
#endif
#include "../src/nn/_.c"
#include "../src/nn/datasets.c"
#include "train_helpers.h"
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
#include <time.h>

static Term mkw(TinyHVM*c,Shape s,u32 fi){u32 n=1;for(u32 i=0;i<s.rank;i++)n*=s.dims[i];f32*d=malloc(n*4);f32 b=sqrtf(2.f/(f32)fi);for(u32 i=0;i<n;i++)d[i]=b*((f32)rand()/(f32)RAND_MAX*2-1)*.5f;Term t=thvm_tensor(c,d,s);free(d);return t;}
static Term mkz(TinyHVM*c,u32 n){f32*z=calloc(n,4);Term t=thvm_tensor(c,z,SHAPE(n));free(z);return t;}
static double now_s(void){struct timespec t;clock_gettime(CLOCK_MONOTONIC,&t);return t.tv_sec+t.tv_nsec*1e-9;}

int main(void) {
    srand(42);
    MNISTData data = mnist_load("data");
    TinyHVM *ctx = thvm_init("metal");
    u32 BS = 64, ff = 784;

    Term w = mkw(ctx, SHAPE(ff, 10), ff);
    Term b = mkz(ctx, 10);
    thvm_set_requires_grad(ctx, w);
    thvm_set_requires_grad(ctx, b);

    // Fixed buffers
    f32 *xbuf = calloc(BS * 784, 4);
    Term x = thvm_tensor(ctx, xbuf, SHAPE(BS, ff)); free(xbuf);
    thvm_set_requires_grad(ctx, x);
    f32 *ohbuf = calloc(BS * 10, 4);
    Term oh = thvm_tensor(ctx, ohbuf, SHAPE(BS, 10)); free(ohbuf);
    f32 lr_val = 0.0001f;  // smaller LR to prevent exploding logits
    Term lr = thvm_tensor(ctx, &lr_val, SHAPE(1));

    #define NP 2
    Term params[NP] = {w, b};
    u32 psz[NP] = {ff*10, 10};
    Term gs[NP];
    for (int i = 0; i < NP; i++) {
        f32 *z = calloc(psz[i], 4);
        gs[i] = thvm_tensor(ctx, z, ctx->tensors[(u32)term_val(params[i])].view.shape);
        free(z);
    }
    for (int i = 0; i < NP; i++) thvm_set_requires_grad(ctx, params[i]);

    // Build graph: dense + softmax + CE loss
    Term logits = thvm_op(ctx, UOP_ADD, thvm_mm(ctx, x, w),
        thvm_expand(ctx, thvm_reshape(ctx, b, SHAPE(1, 10)), SHAPE(BS, 10)));
    Term probs = softmax(ctx, logits, BS, 10);
    f32 eps = 1e-7f;
    Term eps_t = thvm_expand(ctx, thvm_tensor(ctx, &eps, SHAPE(1, 1)), SHAPE(BS, 10));
    Term clamped = thvm_op(ctx, UOP_MAX, probs, eps_t);
    Term log_probs = thvm_op(ctx, UOP_LOG, clamped, term_era());
    Term masked = thvm_op(ctx, UOP_MUL, oh, log_probs);
    Term sum_all = thvm_sum_axes(ctx, masked, (u32[]){0, 1}, 2);
    Term neg = thvm_op(ctx, UOP_NEG, sum_all, term_era());
    f32 inv_B = 1.f / (f32)BS;
    Term scale = thvm_tensor(ctx, &inv_B, SHAPE(1, 1));
    Term loss = thvm_op(ctx, UOP_MUL, neg, scale);

    Term grad_term = thvm_grad_multi(ctx, loss, params, gs, NP);
    Term sgd = term_era();
    for (int i = NP - 1; i >= 0; i--)
        sgd = thvm_app(ctx, thvm_assign(ctx, params[i],
            thvm_op(ctx, UOP_SUB, params[i],
                thvm_op(ctx, UOP_MUL, lr, gs[i]))), sgd);
    // Instead of single APP, reduce grad first, then check gs, then reduce sgd
    // (to verify ordering)
    Term train_step = thvm_app(ctx, grad_term, sgd);
    // Alternative: separate reduction to check ordering
    // thvm_reduce(ctx, grad_term);
    // check gs here
    // thvm_reduce(ctx, sgd);

    u32 n_persistent = ctx->tensor_count;
    printf("n_persistent=%u, pool_count=%u\n", n_persistent, metal_pool.count);

    // Step 0: capture
    u32 x_bid = (u32)term_val(x), oh_bid = (u32)term_val(oh);
    u32 bi = 0;
    ctx->tensors[x_bid].backend->buf_write(ctx->tensors[x_bid].buf_id,
        &data.train_images[bi*BS*784], BS*784*sizeof(f32));
    f32 *oh_data = calloc(BS*10, sizeof(f32));
    for (u32 i = 0; i < BS; i++) oh_data[i*10+data.train_labels[bi*BS+i]] = 1.f;
    ctx->tensors[oh_bid].backend->buf_write(ctx->tensors[oh_bid].buf_id, oh_data, BS*10*sizeof(f32));
    for (int i = 0; i < NP; i++) {
        u32 gid = (u32)term_val(gs[i]);
        memset(metal_pool.bufs[ctx->tensors[gid].buf_id].contents, 0, psz[i]*sizeof(f32));
    }

    jit_begin_capture(n_persistent);
    double t0 = now_s();
    thvm_reduce(ctx, train_step);
    f32 lv = thvm_to_host(ctx, loss)[0];
    jit_end_capture();
    jit_flush();
    printf("capture: loss=%.4f (%.0fms)\n", lv, (now_s()-t0)*1000);

    // Check grads after compute
    for (int i = 0; i < NP; i++) {
        u32 gid = (u32)term_val(gs[i]);
        u32 gbid = ctx->tensors[gid].buf_id;
        f32 *gd = (f32*)metal_pool.bufs[gbid].contents;
        f32 sum = 0;
        for (u32 j = 0; j < psz[i]; j++) sum += fabsf(gd[j]);
        printf("  gs[%d] |grad|_1 = %e (buf %u)\n", i, sum, gbid);
    }
    u32 wbid = ctx->tensors[(u32)term_val(w)].buf_id;
    f32 *wd = (f32*)metal_pool.bufs[wbid].contents;
    printf("  w[2706] after capture: %e\n", wd[2706]);
    for (u32 ci = 0; ci < jit.n_cmds; ci++) {
        JITCmd *cmd = &jit.cmds[ci];
        if (cmd->is_blit) {
            printf("  cmd %2u: BLIT s%u(b%u) ← s%u(b%u) %llu B\n", ci,
                   cmd->blit_dst_slot, jit.slots[cmd->blit_dst_slot].buf_id,
                   cmd->blit_src_slot, jit.slots[cmd->blit_src_slot].buf_id, cmd->blit_size);
        } else if (cmd->is_mps) {
            printf("  cmd %2u: MPS s%u ← s%u,s%u [%u,%u,%u]\n", ci,
                   cmd->mps_dst_slot, cmd->mps_a_slot, cmd->mps_b_slot,
                   cmd->mps_m, cmd->mps_k, cmd->mps_n);
        } else {
            printf("  cmd %2u: COMP bufs=[", ci);
            for (u32 i = 0; i < cmd->n_bufs; i++)
                printf("s%u(b%u)%s", cmd->buf_slots[i], jit.slots[cmd->buf_slots[i]].buf_id,
                       i<cmd->n_bufs-1?",":"");
            printf("] g=%u,%u,%u\n", cmd->grid[0], cmd->grid[1], cmd->grid[2]);
        }
    }

    extern u32 total_dispatches; total_dispatches = 0;
    thvm_reset(ctx, n_persistent);

    // Replay 5 steps — random batches
    for (u32 step = 1; step <= 5; step++) {
        jit_flush();
        bi = rand() % (data.n_train / BS);
        ctx->tensors[x_bid].backend->buf_write(ctx->tensors[x_bid].buf_id,
            &data.train_images[bi*BS*784], BS*784*sizeof(f32));
        memset(oh_data, 0, BS*10*sizeof(f32));
        for (u32 i = 0; i < BS; i++) oh_data[i*10+data.train_labels[bi*BS+i]] = 1.f;
        ctx->tensors[oh_bid].backend->buf_write(ctx->tensors[oh_bid].buf_id, oh_data, BS*10*sizeof(f32));
        for (int i = 0; i < NP; i++) {
            u32 gid = (u32)term_val(gs[i]);
            memset(metal_pool.bufs[ctx->tensors[gid].buf_id].contents, 0, psz[i]*sizeof(f32));
        }
        // Verify batch data was written
        {
            f32 *xd = (f32*)metal_pool.bufs[ctx->tensors[x_bid].buf_id].contents;
            f32 xsum = 0; for (u32 j = 0; j < BS*784; j++) xsum += xd[j];
            printf("  step %u batch: bi=%u xsum=%.2f oh[0]=%u\n", step, bi, xsum,
                   (u32)((f32*)metal_pool.bufs[ctx->tensors[oh_bid].buf_id].contents)[0]);
        }
        jit_replay();
        jit_flush();
        // Check forward MM output (slot 15 = cmd 0 output)
        {
            u32 mm_bid = jit.slots[15].buf_id;
            f32 *mm_data = (f32*)metal_pool.bufs[mm_bid].contents;
            f32 mm_sum = 0; for (u32 j = 0; j < 640; j++) mm_sum += mm_data[j];
            printf("  mm_output sum=%.4f [0:3]=[%.4f,%.4f,%.4f]\n", mm_sum, mm_data[0], mm_data[1], mm_data[2]);
        }
        u32 gw_bid = ctx->tensors[(u32)term_val(gs[0])].buf_id;
        f32 *gd = (f32*)metal_pool.bufs[gw_bid].contents;
        u32 w_bid = ctx->tensors[(u32)term_val(w)].buf_id;
        f32 *wd = (f32*)metal_pool.bufs[w_bid].contents;
        f32 gsum = 0; for (u32 j = 0; j < psz[0]; j++) gsum += fabsf(gd[j]);
        // Find max weight change
        static f32 prev_w[7840]; static int has_prev = 0;
        f32 max_change = 0; u32 change_idx = 0;
        if (has_prev) {
            for (u32 j = 0; j < psz[0]; j++) {
                f32 d = fabsf(wd[j] - prev_w[j]);
                if (d > max_change) { max_change = d; change_idx = j; }
            }
        }
        memcpy(prev_w, wd, psz[0]*4); has_prev = 1;
        // Find max grad
        f32 gmax = 0; u32 gmax_idx = 0;
        for (u32 j = 0; j < psz[0]; j++) { if (fabsf(gd[j]) > gmax) { gmax = fabsf(gd[j]); gmax_idx = j; } }
        printf("step %u: |grad|=%.2f max_grad@%u=%.4f max_w_change@%u=%.6f\n",
               step, gsum, gmax_idx, gd[gmax_idx], change_idx, max_change);
    }

    free(oh_data);
    thvm_free(ctx);
    return 0;
}
