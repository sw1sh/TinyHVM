// test_kernel_out_sq_ref.m — self-MUL on a kernel output that does NOT
// have a reduced (size-1 stride-0) dim. Uses MUL(w_bc, two_bc) where
// both are [M, N] — elementwise, no reduction. Then self-MUL.
//
// If PASSES: the bug is specifically the reduced-dim stride-0 view
//            from SUM output. Self-MUL on elementwise kernel outputs
//            is fine.
// If FAILS: any kernel-output TOP going through self-MUL + MAT-CTR
//           defer + REF fails, regardless of reduction.
...
int main(int argc, char **argv) {
    ...
    Term two = thvm_tensor(ctx, &two_scalar, SHAPE(1));
    ...
    // Forward: w -> expand [M,N] -> MUL by 2_bc [M,N] (elementwise, no
    // reduction) -> self-MUL -> sum -> loss.
    Term w_bc = thvm_expand(ctx, thvm_reshape(ctx, w_fwd, SHAPE(1, N)), SHAPE(M, N));
    Term two_bc = thvm_expand(ctx, two, SHAPE(M, N));
    Term doubled = thvm_op(ctx, UOP_MUL, w_bc, two_bc);   // elementwise [M,N]
    Term sq = thvm_op(ctx, UOP_MUL, doubled, doubled);
    ...
    // Expected: doubled[i, j] = 2 * w[j]. sq[i, j] = (2*w[j])^2.
    // loss = sum over all i, j = M * sum_j (2*w[j])^2 = 4M * sum_j w[j]^2.
    // d/dw[j] = 4M * 2*w[j] = 8*M*w[j].
    f32 exp_gw[N];
    for (u32 j = 0; j < N; j++) exp_gw[j] = 8.0f * (f32)M * wd[j];
