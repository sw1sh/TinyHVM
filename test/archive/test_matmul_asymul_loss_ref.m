// test_matmul_asymul_loss_ref.m — matmul + MUL(pred, c) where c is a
// constant (non-grad) tensor. Not self-MUL — different args.
//
// Loss = sum(pred * c).  gy for pred = c (per-element).
// No SUB, no target, no self-MUL(X, X).
//
// If PASSES: self-MUL(X, X) is specifically what breaks with matmul
//            + MAT-CTR defer.
// If FAILS: any MUL backward that produces a per-element gy going
//           into matmul-decomp backward fails.
...
int main(int argc, char **argv) {
    ...
    f32 cd[M*N] = {  0.1f,  0.2f,  0.3f,  0.4f,
                     0.5f,  0.6f,  0.7f,  0.8f };
    f32 wd[K*N] = {
    ...
    Term c = thvm_tensor(ctx, cd, SHAPE(M, N));
    ...
    // Forward: pred = matmul(x, w) + b,  loss = sum(pred * c)
    Term pred = thvm_op(ctx, UOP_ADD,
        thvm_mm(ctx, x, w_fwd),
        thvm_expand(ctx, thvm_reshape(ctx, b_fwd, SHAPE(1, N)), SHAPE(M, N)));
    Term weighted = thvm_op(ctx, UOP_MUL, pred, c);
    Term loss = thvm_sum_axes(ctx, weighted, (u32[]){0, 1}, 2);
    ...
    // Expected: gy for pred = c (from MUL(pred, c) backward).
    // gw[k, j] = sum_i (c[i, j] * x[i, k])
    // gb[j]   = sum_i (c[i, j])
    f32 exp_gw[K*N] = {0}, exp_gb[N] = {0};
    for (u32 k = 0; k < K; k++)
        for (u32 j = 0; j < N; j++) {
            f32 s = 0;
            for (u32 i = 0; i < M; i++) s += cd[i*N + j] * xd[i*K + k];
            exp_gw[k*N + j] = s;
        }
    for (u32 j = 0; j < N; j++) {
        f32 s = 0;
        for (u32 i = 0; i < M; i++) s += cd[i*N + j];
        exp_gb[j] = s;
    }
