// test_matmul_dupmul_loss_ref.m — matmul + explicit DUP of pred before
// MUL, instead of MUL(pred, pred). Forward uses thvm_dup to split pred
// into two distinct DP tokens, then MUL(dp0, dp1).
//
// If PASSES: the bug is when MUL's two args are the SAME term value
//            (implicit sharing). Passing explicit DUP solves it.
// If FAILS: the bug is in the DUP-of-matmul-chain commute pattern,
//           not in the "same term value" sharing.
...
int main(int argc, char **argv) {
    ...
    // Forward: pred = matmul(x, w) + b, then EXPLICITLY DUP pred so the
    // MUL's two args are distinct DP tokens instead of the same term.
    Term pred = thvm_op(ctx, UOP_ADD,
        thvm_mm(ctx, x, w_fwd),
        thvm_expand(ctx, thvm_reshape(ctx, b_fwd, SHAPE(1, N)), SHAPE(M, N)));
    Term pred0, pred1;
    thvm_dup(ctx, thvm_fresh_label(ctx), pred, &pred0, &pred1);
    Term sq = thvm_op(ctx, UOP_MUL, pred0, pred1);
    ...
    // Expected: loss = sum(pred*pred), gy for pred = 2*pred.
