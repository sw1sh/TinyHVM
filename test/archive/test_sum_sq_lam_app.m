// test_sum_sq_lam_app.m — same body structure but invoked via direct
// LAM-APP, no REF/ALO indirection. Body is: λw_p. sum(pooled * pooled)
// where pooled = sum(expand(w_p)). Called with w_fwd as arg.
//
// If PASSES: the bug is specifically ALO realization (REF->dyn), not
//            LAM substitution itself.
// If FAILS: any lambda substitution through this forward expression
//           triggers the collapse — not specific to ALO.
...
int main(int argc, char **argv) {
    ...
    // Build LAM body in dyn heap (no REF/book).
    ...
    // Direct LAM-APP, no REF.
    Term loss = thvm_app(ctx, lam, w_fwd);
