// test_sum_sq_ref_scale.m — exact copy of test_sum_sq_ref (which PASSES
// with the current fix) but with a trailing outer MUL by constant scalar
// applied to the loss (just like matmul_ref_noassign's /N factor).
//
// test_sum_sq_ref: grad-inside-REF with MAT-CTR; self-MUL of SUM output;
// PASSES per-element correctly.
//
// Adds: loss = loss * inv_n where inv_n = 0.5 (constant [1,1]).
//
// Expected gw = 2*M^2*w[j] * 0.5 = M^2*w[j].
// For M=2, w=[0.1,-0.2,0.3,0.4]: exp = [0.4, -0.8, 1.2, 1.6].
//
// If PASSES: outer-MUL-with-const works fine in sum_sq_ref structure.
//   Half-magnitude bug needs something else (matmul-specific).
// If FAILS half (gw = [0.2,-0.4,0.6,0.8]): the outer MUL with const
//   halves gradient in ANY grad-inside-REF structure. Root cause
//   isolated to this pattern.
...
int main(int argc, char **argv) {
    ...
    // CRITICAL: add trailing MUL(loss, 0.5) with constant scalar.
