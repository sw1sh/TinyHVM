// test_tiny_twoparam_linear_loop.m — elementwise linear regression SGD loop
//
// train(counter)(w)(b) =
//   IFZ(counter,
//       CTR(w, b),
...
//
// loss(w, b) = sum((w*x + b - target)^2)
//
// Step up from test_tiny_twoparam_grad_loop:
//  - adds a feature tensor x (not learned) inside the forward
//  - MUL backward now asymmetric: dL/dw = 2*(pred - target) * x
//    (left arg = diff, right arg = x — different sources)
//  - dL/db = 2*(pred - target) as before
//
// Elementwise only (no matmul). Asserts exact SGD trajectory at
// n=1, 2, 3 against a host-simulated reference.
...
static Term build_loss(TinyHVM *ctx, Term w, Term b, Term x, Term target) {
    Term pred = thvm_op(ctx, UOP_ADD, thvm_op(ctx, UOP_MUL, w, x), b);
    Term diff = thvm_op(ctx, UOP_SUB, pred, target);
    Term sq = thvm_op(ctx, UOP_MUL, diff, diff);
    return thvm_sum_axes(ctx, sq, (u32[]){0}, 1);
}

static Term recursive_train(TinyHVM *ctx,
                            Term w0, Term b0,
                            Term x, Term target,
                            Term lr, u32 n_steps) {
    ...
    Term loss = build_loss(ctx, w_fwd, b_fwd, x, target);
...
static void simulate(f32 *w, f32 *b,
                     const f32 *x, const f32 *target,
                     f32 lr, u32 n_steps) {
    for (u32 step = 0; step < n_steps; step++) {
        f32 diff[N];
        for (u32 i = 0; i < N; i++) diff[i] = (w[i] * x[i] + b[i]) - target[i];
        for (u32 i = 0; i < N; i++) {
            f32 g = 2.0f * diff[i];
            w[i] -= lr * g * x[i];
            b[i] -= lr * g;
...
int main(int argc, char **argv) {
    ...
    f32 xd[N] = { 2.0f,  0.5f, -1.5f };
    f32 td[N] = { 1.2f,  0.3f,  0.8f };
    f32 lrd[] = { 0.05f };

    Term w = thvm_tensor(ctx, wd, SHAPE(N));
    Term b = thvm_tensor(ctx, bd, SHAPE(N));
    Term x = thvm_tensor(ctx, xd, SHAPE(N));
    ...
    Term program = recursive_train(ctx, w, b, x, target, lr, n_steps);
    ...
    simulate(ew, eb, xd, td, lrd[0], n_steps);
    ...
    assert(all_match && "elementwise linear SGD must match host simulation");
    printf("PASS\n");
