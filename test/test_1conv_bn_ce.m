...
static void test(const char *dev){
    ...
    Term bn_g=mkones(ctx,4),bn_b=mkz(ctx,4),bn_rm=mkz(ctx,4),bn_rv=mkones(ctx,4);
    u32 ff=4*12*12; Term lw=mkw(ctx,SHAPE(ff,10),ff),lb=mkz(ctx,10);
    thvm_set_requires_grad(ctx,cw1);thvm_set_requires_grad(ctx,cb1);
    thvm_set_requires_grad(ctx,bn_g);thvm_set_requires_grad(ctx,bn_b);
    thvm_set_requires_grad(ctx,lw);thvm_set_requires_grad(ctx,lb);
    ...
    // Conv1(k5)→ReLU→BN→Pool→Dense→CE
    Term h=thvm_conv2d(ctx,x,cw1,cb1,1,s1,p0); // [4,4,24,24]
    h=thvm_op(ctx,UOP_RELU,h,term_era());
    BNResult bn=batchnorm_forward(ctx,h,bn_g,bn_b,bn_rm,bn_rv,BS,4,24,24,1);
    h=bn.output;
    h=thvm_maxpool2d(ctx,h,k2,s2); // [4,4,12,12]
    ...
    printf("%-6s: ",dev);
    const char*n[NP]={"cw1","cb1"};
    for(int i=0;i<NP;i++){f32*gd=thvm_to_host(ctx,gs[i]);double nm=0;
        for(u32 j=0;j<psz[i];j++)nm+=gd[j]*gd[j];printf("%s=%.4f ",n[i],sqrt(nm));}
    printf("\n");thvm_free(ctx);
    #undef NP
}
