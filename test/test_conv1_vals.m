...
int main(void){
    ...
    u32 OH=H-K+1;
    Term flat=thvm_reshape(ctx,h,SHAPE(BS*Cout*OH*OH));
    f32 *dst=calloc(BS*Cout*OH*OH,4);
    Term dt=thvm_tensor(ctx,dst,SHAPE(BS*Cout*OH*OH));
    ...
    double sum=0;for(u32 i=0;i<BS*Cout*OH*OH;i++)sum+=r[i];
    printf("loss=%.4f h[0,0,0,:3]=[%.7f,%.7f,%.7f]\n",sum,r[0],r[1],r[2]);
