...
static void test(const char *dev) {
    ...
    // Use MUL(bn_output, constant) instead of MUL(bn_output, bn_output)
    // to avoid DUP
    f32 two = 2.0f;
    Term scaled = thvm_op(ctx, UOP_MUL, bn.output, thvm_tensor(ctx, &two, SHAPE(1)));
    extern Term thvm_sum_axes(TinyHVM*,Term,const u32*,u32);
    Term loss=thvm_sum_axes(ctx,scaled,(u32[]){0,1,2,3},4);
    ...
    printf("%-6s: dw_norm=%.4f dw[0..3]=[%.6f,%.6f,%.6f,%.6f]\n",
