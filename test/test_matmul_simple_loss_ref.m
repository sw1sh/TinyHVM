// test_matmul_simple_loss_ref.m — minimal matmul backward through
// MAT-CTR defer + REF, without any diff/square loss math.
// Loss = sum(matmul(x, w) + bias).  No squaring, no target.
//
// Expected: gw[k, j] = sum_i(x[i, k]) for all j (uniform across j).
//           gb[j]   = M for all j (sum over i of gy=1).
//
// This IS the shape of "uniform across j" but the VALUE per row should
// be sum_i(x[i,k]), not the diff[0,0]/4 value we see in the failing
// linear_sgd_loop case.
//
// If gw rows match sum_i(x[i,k]): matmul-decomp backward through
// MAT-CTR defer works when there's no MUL(diff,diff) chain.
//
// If gw is wrong (some other scalar pattern): the bug is in matmul
// backward alone through MAT-CTR defer, independent of diff squaring.
...
int main(int argc, char **argv) {
    ...
    // Simplified forward: pred = matmul(x, w) + b.  No diff, no square.
    Term pred = thvm_op(ctx, UOP_ADD,
        thvm_mm(ctx, x, w_fwd),
        thvm_expand(ctx, thvm_reshape(ctx, b_fwd, SHAPE(1, N)), SHAPE(M, N)));
    Term loss = thvm_sum_axes(ctx, pred, (u32[]){0, 1}, 2);
    ...
    // Expected: gw[k, j] = sum_i(x[i, k]); gb[j] = M (sum over i of gy=1).
    f32 exp_gw[K*N] = {0}, exp_gb[N] = {0};
    for (u32 k = 0; k < K; k++) {
        f32 s = 0;
        for (u32 i = 0; i < M; i++) s += xd[i*K + k];
        for (u32 j = 0; j < N; j++) exp_gw[k*N + j] = s;
    }
    for (u32 j = 0; j < N; j++) exp_gb[j] = (f32)M;
