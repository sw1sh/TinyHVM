// test_matmul_sub_selfmul_scale.m — identical to test_matmul_sub_selfmul
// but adds a trailing MUL(loss, inv_n) scaling (inv_n is a [1,1] constant).
//
// If PASSES (scaled properly): outer-MUL-with-constant is not the trigger.
// If FAILS half-magnitude: the outer MUL with a constant TEN halves the
//   gradient somewhere — isolates that step as the root cause of the
//   half-magnitude in test_matmul_grad_mat_ref_noassign.
...
int main(int argc, char **argv) {
    ...
    // CRITICAL ADDITION: scale loss by constant 1/8 via outer MUL.
    ...
    // Expected (with /8): scale all gradients by 1/8
    ...
    for (u32 k = 0; k < K; k++)
        for (u32 j = 0; j < N; j++) {
            ...
            exp_gw[k*N + j] = s * (1.0f / 8.0f);
        }
    for (u32 j = 0; j < N; j++) {
        f32 s = 0;
        for (u32 i = 0; i < M; i++) s += 2.0f * diff_host[i*N + j];
        exp_gb[j] = s * (1.0f / 8.0f);
    }
