...
int main(void) {
    ...
    printf("  Training: %u epochs × %u batches (BS=%u), lr=%.4f with decay\n", n_epochs, n_batches, BS, lr);
    ...
    for (u32 epoch = 0; epoch < n_epochs; epoch++) {
        // Learning rate decay
        if (epoch == 1) lr = 0.0005f;
        if (epoch == 2) lr = 0.0002f;
          ...
          @autoreleasepool {
            ...
            if (step < 5 || step % 100 == 0 || bi_idx == n_batches - 1) {
                ...
                if (step < 5 || step % 200 == 0)
                    printf("  epoch %u step %4u/%u: loss=%.4f lr=%.4f\n",
                           epoch, bi_idx, n_batches, lv, lr);
            }
