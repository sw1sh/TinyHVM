// test_sum_dupmul_grad_ref.m — same REF+grad structure as test_sum_sq_ref
// but with EXPLICIT user DUP of pooled before MUL. In test_sum_sq_ref the
// forward is MUL(pooled, pooled) where at==bt literally. Here we split
// pooled via user DUP first: MUL(dp0, dp1) where dp0 != dp1 as terms.
//
// If PASSES: bug is specifically MUL(same-term, same-term) through ALO
//            in the grad path (BG rule's at==bt branch + ALO realize).
// If FAILS:  bug is any DUP-of-SUM through ALO in backward, independent
//            of whether the two MUL operands share a term or not.
...
int main(int argc, char **argv) {
    ...
    // Then EXPLICIT user DUP of pooled → (dp0, dp1), then MUL(dp0, dp1)
    // (vs test_sum_sq_ref's MUL(pooled, pooled) with shared term).
