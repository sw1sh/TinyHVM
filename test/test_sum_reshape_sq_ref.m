// test_sum_reshape_sq_ref.m — same as test_sum_sq_ref but with a
// RESHAPE between SUM and self-MUL that squeezes the reduced axis.
// Forward: w -> expand [M,N] -> sum axis 0 -> [1,N] -> RESHAPE to [N]
//          -> self-MUL -> sum -> loss.
//
// If PASSES: the singleton [1,N] view from SUM is what causes the
//            scalar-collapse during DUP commute. Squashing to [N]
//            fixes it.
// If FAILS: the SUM + self-MUL pair fails regardless of view shape,
//           pointing at DUP⊳SUM or DUP⊳reshape-of-SUM commute.
...
int main(int argc, char **argv) {
    ...
    // Forward: expand(w) [M,N] -> sum axis 0 -> [1,N] -> RESHAPE to [N]
    //          -> self-MUL -> sum -> loss.
    Term w_bc = thvm_expand(ctx, thvm_reshape(ctx, w_fwd, SHAPE(1, N)), SHAPE(M, N));
    Term pooled = thvm_sum_axes(ctx, w_bc, (u32[]){0}, 1);     // [1, N]
    Term squeezed = thvm_reshape(ctx, pooled, SHAPE(N));        // [N]
    Term sq = thvm_op(ctx, UOP_MUL, squeezed, squeezed);
    ...
    // Same expected as test_sum_sq_ref: gw[j] = 2*M^2*w[j]
    f32 exp_gw[N];
