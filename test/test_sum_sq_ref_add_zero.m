// test_sum_sq_ref_add_zero.m — same as test_sum_sq_ref_scale but with
// ADD(loss, 0.0) identity op instead of MUL(loss, 0.5). d(loss+0)/d(w)
// should give the same gradient as no ADD.
//
// ADD backward: d(ADD(a,b))/da = gy, d(ADD(a,b))/db = gy. For non-req-grad
// const b, b-arm returns 0. a-arm propagates full gy.
//
// If PASSES with correct values (no halving): halving is MUL-specific.
// If FAILS half: halving is general binary-op-with-const.
...
int main(int argc, char **argv) {
    ...
    // ADD with zero constant — identity in forward, gy passthrough in backward.
    Term zero_const = thvm_reshape(ctx, thvm_scalar(ctx, 0.0f), SHAPE(1, 1));
    loss = thvm_op(ctx, UOP_ADD, loss, zero_const);
    ...
    // Expected (ADD with 0 is identity, same as no-scale): 2*M^2*w[j] = 8*w[j]
    f32 exp_gw[N];
