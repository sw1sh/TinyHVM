// test_sum_sq_ref_scale_2x.m — test_sum_sq_ref_scale but with const=2.0.
// Same structure (outer MUL(loss, const) in REF body + MAT-CTR).
//
// No-scale expected: gw[j] = 2*M^2*w[j] = 8*w[j] = [0.8, -1.6, 2.4, 3.2].
// With const=2.0: expected gw[j] = 2*8*w[j] = 16*w[j] = [1.6, -3.2, 4.8, 6.4].
//
// If observed = [0.8, -1.6, 2.4, 3.2]: ratio 0.5 — halved same as 0.5 case.
// If observed = [1.6, -3.2, 4.8, 6.4]: full magnitude — const=0.5 specific.
// If observed = [3.2, -6.4, 9.6, 12.8]: doubled — const applied twice.
...
int main(int argc, char **argv) {
    ...
    Term two_const = thvm_reshape(ctx, thvm_scalar(ctx, 2.0f), SHAPE(1, 1));
    loss = thvm_op(ctx, UOP_MUL, loss, two_const);
    ...
    f32 exp_gw[N];
    for (u32 j = 0; j < N; j++) exp_gw[j] = 2.0f * (f32)M * (f32)M * wd[j] * 2.0f;
