// test_sum_sq_ref_scale_noreshape.m — variant of test_sum_sq_ref_scale.
// The const scalar is built with thvm_tensor (shape [1]) directly
// instead of thvm_scalar + thvm_reshape, avoiding a RESHAPE TOP
// wrapping the constant inside the REF body. Also no outer
// reshape(loss, [1,1]).
//
// test_sum_sq_ref_scale FAILS half because RESHAPE TOP isn't in my
// GRAD_ALO_SAFE_TO_FORCE gate (RESHAPE excluded — triggers matmul
// crash). So bt = ALO(RESHAPE(TEN)) isn't forced, stays ALO.
//
// If this PASSES: the half-magnitude is specifically the RESHAPE-ALO
//   wrapping of the constant bt. Fix direction: make bt force peek
//   through RESHAPE if its grandchild is a TEN.
// If this still FAILS: RESHAPE isn't the trigger; something else in
//   how gy propagates through DUP in outer MUL is the bug.
...
int main(int argc, char **argv) {
    ...
    // Use shape-[1] constant directly (no RESHAPE around it).
    // loss is shape [1] from sum_axes.
    Term half_const = thvm_tensor(ctx, &half_data, SHAPE(1));
    ...
    if (!gwf) { printf("FAIL: gw not readable — NUM(0) zero-grad path\n"); return 1; }
