...
TINetGraph::usage = "TINetGraph[tensor|term|snapshot] renders the interaction-net graph from the pure-WL heap walk rooted at the given term — boxed nodes with per-tag shapes, port labels, heap-location tails, and the next active pair highlighted in red when one exists. Accepts a live term, a walk snapshot (from TStepTrace), or a raw walker association with \"Nodes\" and \"Edges\".";
TINetGraphDOT::usage = "TINetGraphDOT[tensor|term|snapshot] returns a DOT string generated from the same pure-WL heap walk data used by TINetGraph.";
TINetGraphImport::usage = "TINetGraphImport[tensor|term|snapshot] renders the pure-WL heap-walk DOT generated by TINetGraphDOT through ImportDOTString. ImportDOTString options can be passed through.";
TBitField::usage = "TBitField[{{name, bits, value}, ...}] renders a 64-bit term word as a row of proportional labeled boxes (SUB / TAG / EXT / VAL by convention). Narrow fields get a minimum visual width so their labels remain legible. Options: \"ImageSize\" (default 640).";
TShapeLegend::usage = "TShapeLegend[] renders a horizontal gallery of the node shapes used by TINetGraph (LAM triangle, APP inv-triangle, SUP hexagon, VAR oval, NUM/OP2 rectangles, ERA dot) colored to match the interaction-net renderer.";
THeapStrip::usage = "THeapStrip[] returns a horizontal strip of the current heap slots as colored cells (tag + val), one cell per word. Useful as a compact complement to TINetGraph. THeapStrip[lo, hi] restricts to slots lo..hi.";
